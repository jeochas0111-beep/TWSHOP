...
// 获取订单项的照片列表
router.get('/production-photos/:orderItemId', route((req, res) => {
  const orderItemId = Number(req.params.orderItemId);
  if (!Number.isFinite(orderItemId) || orderItemId <= 0) return notFound('订单项不存在');
  const item = db.prepare('SELECT id FROM order_items WHERE id=?').get(orderItemId);
  if (!item) return notFound('订单项不存在');
  const photos = db.prepare('SELECT * FROM production_photos WHERE order_item_id=? ORDER BY created_at DESC').all(orderItemId);
  res.json(photos);
}));

// 上传照片（支持多图）
router.post('/production-photos/:orderItemId', (req, res, next) => {
  ...
  const insert = db.prepare('INSERT INTO production_photos(order_item_id, filename, original_name, mime_type, size_bytes, created_at) VALUES(?,?,?,?,?,?)');
  const photos = [];
  for (const file of req.files) {
    const info = insert.run(orderItemId, file.filename, file.originalname, file.mimetype, file.size, now());
