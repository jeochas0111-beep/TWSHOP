...
function requireChannelOperator(req, res, next) {
  if (appConfig.authConfig().noAuth) return next();
  if (req.user?.role === 'admin') return next();
  if (req.user?.role === 'operator' && req.user?.channel === req.channel) return next();
  return sendError(res, 403, '没有当前渠道的操作权限');
}
