...
const VALID_TABLES = new Set(['globals','fabrics','linings','products','product_width_prices','product_length_prices','product_option_groups','product_option_values','product_archive','labor_rules','memory_rules','tax_rates','orders','order_items','factory_feedback','backups','users']);
...
function importBackupPayload(payload) {
  ...
  const tx = db.transaction(() => {
    for (const table of ['factory_feedback', 'order_items', 'orders', 'product_option_values', 'product_option_groups', 'product_width_prices', 'product_length_prices', 'product_archive', 'products', 'fabrics', 'linings', 'labor_rules', 'memory_rules', 'tax_rates', 'globals']) db.prepare(`DELETE FROM ${table}`).run();
