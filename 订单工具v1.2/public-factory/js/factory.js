...
function unifiedLoginUrl() {
  return APP_CONTEXT.loginUrl || `${APP_CONTEXT.portalOrigin || ""}${APP_CONTEXT.loginPath || "/login"}` || "/login";
}
...
async function loadUsers(channelFilter) {
  try {
    ...
    const body = rows.map((u) => `<tr>
      ...
      <td>${esc(userRoleLabel(u.role))}</td>
      <td><code>${esc(userWorkbenchLabel(u))}</code></td>
      <td>${u.enabled ? '<span class="pill good">启用</span>' : '<span class="pill bad">停用</span>'}</td>
    </tr>`);
    renderTable("usersTable", ["ID", "账户归属", "显示名", "用户名", "角色", "默认入口", "状态"], body);
  } catch (e) {
    toast(e.message, "bad");
  }
}
  ...
  try {
    ...
    content.innerHTML =
      ...
      (order.delivery_screenshot
        ? '<div class="delivery-screenshot-section"><div class="delivery-screenshot-header"><h4>签收截图</h4></div><div class="delivery-screenshot-area"><div class="delivery-screenshot-preview"><img data-auth-src="/api/orders/' + order.id + '/delivery-screenshot" alt="签收截图"></div></div></div>'
        : '') +
      '</div></div>' +
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $("summaryExportBtn")?.addEventListener("click", () => {
    ...
    location.href = apiUrl("/api/export/summary-csv") + (params.toString() ? "?" + params : "");
  });
  $("summaryFullExportBtn")?.addEventListener("click", async () => {
    try {
      const res = await fetch(apiUrl('/api/export/orders-full-batch'), {
        method: 'POST',
