...
function isManagementChannel(channel) {
  return channel === 'management';
}

function getRequestChannel(req) {
  return req.channel || 'shopify';
}

function assertOrderChannelAccess(req, orderOrChannel) {
  const channel = getRequestChannel(req);
  const targetChannel = typeof orderOrChannel === 'string' ? orderOrChannel : orderOrChannel?.channel;
  if (isManagementChannel(channel)) return;
  if (targetChannel && targetChannel !== channel) notFound('订单不存在');
}

function getOrderChannelByItemId(itemId) {
  return db.prepare(`
    SELECT o.channel
    FROM order_items oi
...
router.get('/orders', (req, res) => {
  const channel = getRequestChannel(req);
  const isManagement = isManagementChannel(channel);
  const rows = isManagement
    ? db.prepare('SELECT * FROM orders ORDER BY order_date DESC, created_at DESC, id DESC LIMIT 200').all()
    : db.prepare('SELECT * FROM orders WHERE channel=? ORDER BY order_date DESC, created_at DESC, id DESC LIMIT 200').all(channel);
...
router.get('/orders/:id', route((req, res) => {
  ...
  assertOrderChannelAccess(req, o);
  res.json(o);
}));

router.post('/orders', route((req, res) => {
  if (!req.body || typeof req.body !== 'object') badRequest('请求体必须是 JSON 对象');
  sendOk(res, { order: saveOrder({ ...req.body, channel: getRequestChannel(req) }) });
}));
...
router.put('/orders/:id', route((req, res) => {
  ...
  assertOrderChannelAccess(req, existing);
...
router.put('/orders/:id/status', route((req, res) => {
  ...
  const existing = db.prepare('SELECT channel FROM orders WHERE id=?').get(id);
  if (!existing) notFound('订单不存在');
  assertOrderChannelAccess(req, existing);
...
router.post('/orders/:id/recalculate', route((req, res) => {
  const existing = db.prepare('SELECT channel FROM orders WHERE id=?').get(req.params.id);
  if (!existing) notFound('订单不存在');
  assertOrderChannelAccess(req, existing);
...
router.put('/orders/:id/cost-overrides', route((req, res) => {
  const id = positiveIntParam(req, 'id', '订单不存在');
  const existing = db.prepare('SELECT id,channel FROM orders WHERE id=?').get(id);
  if (!existing) notFound('订单不存在');
  assertOrderChannelAccess(req, existing);
...
router.put('/orders/:id/financial', route((req, res) => {
  ...
  assertOrderChannelAccess(req, order);
...
router.put('/order-items/:id/discount', route((req, res) => {
  ...
  const itemChannel = getOrderChannelByItemId(itemId);
  if (!itemChannel) notFound('项目不存在');
  assertOrderChannelAccess(req, itemChannel);
...
router.post('/orders/recalculate-all', route((req, res) => {
  ...
  const ids = isManagement
    ? db.prepare('SELECT id FROM orders').all().map((r) => r.id)
    : db.prepare('SELECT id FROM orders WHERE channel=?').all(channel).map((r) => r.id);
  let updated = 0;
  for (const id of ids) {
...
router.get('/orders/:id/delivery-screenshot', route((req, res) => {
  ...
  const order = db.prepare('SELECT delivery_screenshot,channel FROM orders WHERE id=?').get(id);
...
router.post('/orders/:id/delivery-screenshot', (req, res, next) => {
  ...
  const id = positiveIntParam(req, 'id', '订单不存在');
  if (id === undefined) return;
  const order = db.prepare('SELECT id,delivery_screenshot,channel FROM orders WHERE id=?').get(id);
