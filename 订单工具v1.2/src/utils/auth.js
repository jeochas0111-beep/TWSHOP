...
function jwtAuth(channel) {
  return (req, res, next) => {
    ...
    const requestedChannel = channel !== undefined ? channel : req.channel;
    const expectedChannel = requestedChannel === 'management' ? null : requestedChannel;
    const payload = verifyJwt(token, generateSecret());
    if (!payload) {
      return res.status(401).json({ ok: false, error: 'token 无效或已过期' });
    }
    if (expectedChannel != null && payload.channel !== expectedChannel && payload.role !== 'admin') {
      return res.status(401).json({ ok: false, error: 'token 无效或已过期' });
    }
    if (requestedChannel === 'management' && payload.role !== 'admin') {
      return res.status(403).json({ ok: false, error: '当前账号不能访问管理端' });
    }
    req.user = payload;
    next();
  };
